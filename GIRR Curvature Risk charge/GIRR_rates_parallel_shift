...
GIRR Curvature Risk Charge Calculator - Parallel Shift Only
Calculates Base Price, Price Up (+1.7% parallel), Price Down (-1.7% parallel)
...
def calculate_bond_parallel_shift_prices(bond_data, yield_curve_handle, calendar, zero_rates, shift_size=0.017):
    """
    Calculate Base Price, Price Up, and Price Down for parallel shifts
    shift_size: parallel shift amount (default 1.7% = 0.017)
    """
    ...
    # Calculate Price Up (parallel shift up by 1.7%)
    ...
    # Calculate Price Down (parallel shift down by 1.7%)
...
def export_to_excel(results_df, output_file='girr_curvature_parallel_shifts.xlsx'):
    """Export results to Excel with formatting"""
    
    with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
        # Write main data
        results_df.to_excel(writer, sheet_name='GIRR_Curvature_Data', index=False)
        
        # Get workbook and worksheet objects
        workbook = writer.book
        worksheet = writer.sheets['GIRR_Curvature_Data']
...
def run_girr_curvature_analysis(
    rates_file="All Constant Maturity TREas rates.xlsx",
    holdings_file="Bond holdings.xlsx",
    export_file='girr_curvature_parallel_shifts.xlsx'
):
    """
    Main function to run GIRR curvature analysis with parallel shifts only
    """
    print("GIRR Curvature Risk Charge Calculator - Parallel Shifts Only")
    print("Shift Amount: +/- 1.7% (parallel across all tenors)")
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    results = run_girr_curvature_analysis()
    print("\nAnalysis completed!")
