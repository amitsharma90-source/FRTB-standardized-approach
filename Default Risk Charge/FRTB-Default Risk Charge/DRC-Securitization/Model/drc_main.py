...
def run_full_drc(write_phase_snapshots: bool = True) -> dict:
    ...
    # Phases 2-8: chain with snapshots between each phase. The engine returns a
    # result dict {positions, tranches, buckets, total}; sec capital is a
    # BUCKET-level quantity (MAR22.31-35), not a sum of per-position capitals.
    if write_phase_snapshots:
        sec_result = run_engine_with_phase_snapshots(sec_positions, sec_cfg)
    else:
        from drc_securitisation.sec_engine import run_engine as _run_sec
        sec_result = _run_sec(sec_positions, sec_cfg)
    sec_engine_out = sec_result["positions"]
    sec_tranches = sec_result["tranches"]
    sec_buckets = sec_result["buckets"]
    sec_total = float(sec_result["total"])
    ...
    return {
        ...
        "sec_tranches": sec_tranches,
        "sec_buckets": sec_buckets,
        "sec_total": sec_total,
...
def write_combined_output(result: dict, path: str = OUTPUT_PATH) -> None:
    ...
    summary = pd.DataFrame([
        {"line": "DRC SEC non-CTP (MAR22.31-35 + CRE40-45)",
    ...
    # SEC per-bucket view (the authoritative capital level, MAR22.33-35)
    sec_buckets_view = result["sec_buckets"]

    # SEC per-tranche view (collapsed netted frame, phase-6 shape): un-weighted
    # net JTD + RW pass-through. RW is applied (and the HBR netting done) at the
    # bucket level (phase 7, MAR22.25/22.33), so the authoritative capital is the
    # per-bucket DRC (sec_buckets), not a per-tranche figure.
    sec_tranches = result["sec_tranches"][
        ["tranche_key", "Pool ID", "drc_sec_bucket", "approach",
         "is_senior_derived", "Rating",
         "Attachment Pt (%)", "Detachment Pt (%)",
         "rw", "net_jtd_tranche", "abs_net_jtd_tranche"]
    ].sort_values("abs_net_jtd_tranche", ascending=False)

    # SEC per-position view (attribution only — sec capital is bucket-level, so
    # there is no per-position capital column).
    sec_positions_view = result["sec_positions"][
        ["ID", "Issuer", "Security", "Pool ID", "tranche_key", "drc_sec_bucket",
         "approach", "Long/Short", "signed_notional", "MV_USD",
         "maturity_scaled_jtd", "rw", "net_jtd_tranche"]
    ].copy()
    ...
    with pd.ExcelWriter(path, engine="openpyxl") as xl:
        ...
        sec_buckets_view.to_excel(xl, sheet_name="sec_buckets", index=False)
...
def print_summary(result: dict) -> None:
    ...
    print("=" * 72)
    print()
    print(f"  DRC sec non-CTP (MAR22.31-35 + CRE40-45):")
    print(f"    Positions:      {len(result['sec_positions'])}")
    print(f"    Tranches:       {result['sec_positions']['tranche_key'].nunique()}")
    print(f"    Buckets:        {len(result['sec_buckets'])}")
    print(f"    Capital:        ${result['sec_total']:>15,.0f}  (bucket HBR)")
    print()
...
def _list_phase_outputs() -> list[str]:
    """Non-sec (5 phases) + sec (8 phases) + combined workbook this run produces."""
    return [
        ...
        "drc_sec_phase5_gross_scaled_jtd.xlsx",
        "drc_sec_phase6_tranche_net.xlsx",
        "drc_sec_phase7_bucket_hbr.xlsx",
        "drc_sec_phase8_total.xlsx",
