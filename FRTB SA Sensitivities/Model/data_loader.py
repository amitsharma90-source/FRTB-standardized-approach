...
def load_config(filepath: str) -> dict:
    ...
    # Parse CSR non-sec risk weights by bucket (used by callable bond CSR
    # curvature, MAR21.99: parallel spread shock = highest delta RW for bucket;
    # CSR delta RWs are uniform across all CSR tenors so 'highest' = bucket RW)
    cfg['csr_nonsec_rw'] = {}
    if 'CSR_NONSEC_RW' in cfg:
        for _, row in cfg['CSR_NONSEC_RW'].iterrows():
            try:
                cfg['csr_nonsec_rw'][int(row['bucket'])] = float(row['risk_weight'])
            except (ValueError, KeyError):
                pass
