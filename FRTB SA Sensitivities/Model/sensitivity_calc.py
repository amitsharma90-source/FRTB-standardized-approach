...
def calc_callable_bond_csr_curvature(inst, mkt, cfg):
    """
    CSR non-sec curvature for a callable corporate bond [MAR21.97 + MAR21.99].

    Per MAR21.97, delta CSR buckets are replicated for curvature. Per MAR21.99,
    the curvature shock is the parallel shift on the curve based on the highest
    delta risk weight for the bucket. For CSR non-sec the bucket RW is uniform
    across all CSR tenors so the 'highest' is just the bucket RW.

    Mechanics mirror calc_callable_bond_girr_curvature but bump the issuer
    credit spread curve (parallel additive shift) instead of the SOFR yield
    curve. The Hull-White tree reprices the callable under each shocked
    combined-discount-curve (SOFR + bumped spread).

    sum_d per MAR21.5(2)(f)(ii) is the sum of delta sensitivities to all
    tenors of the relevant curve -- here, the sum of CSR CS01s for this bond.
    """
    mat_yrs = years_between(mkt['val_date'], inst['maturity'])
    if mat_yrs <= 0: return {}
    coupon = inst.get('coupon', 0) / 100
    ...
    from data_loader import get_issuer_spread_curve
    spread_curve = get_issuer_spread_curve(
        mkt, inst.get('security', ''),
        inst.get('rating', 'A'), inst.get('issue_type', ''))
    if not spread_curve:
        return {}

    # Curvature RW = highest delta RW for this CSR bucket (uniform across tenors)
    csr_rw_table = cfg.get('csr_nonsec_rw', {})
    curv_rw = csr_rw_table.get(bucket)
    if curv_rw is None:
        return {}

    base_handle = mkt['usd_handle'] if ccy == 'USD' else mkt['gbp_handle']

    def _callable_npv_with_spread_shift(shift):
        """Reprice the callable with the issuer spread curve shifted in
        parallel by `shift` (additive)."""
        bumped_spread = {t: s + shift for t, s in spread_curve.items()}
        combined_handle = _build_ql_spread_curve(
            mkt['ql_val_date'], base_handle, bumped_spread)
        bond, scale, _, _ = _build_ql_callable(
            inst['notional'], coupon, inst['maturity'], inst['call_date'],
            inst.get('freq', 2), combined_handle, cfg, inst.get('issue_date'))
        return bond.NPV() * scale

    pv_base = _callable_npv_with_spread_shift(0)
    pv_up = _callable_npv_with_spread_shift(curv_rw)
    pv_dn = _callable_npv_with_spread_shift(-curv_rw)
    sum_d = sum(calc_callable_bond_csr_delta(inst, mkt, cfg).values())
    return {
        f"CURV_CSR_NONSEC_{bucket}_UP": -(pv_up - pv_base) + curv_rw * sum_d,
        f"CURV_CSR_NONSEC_{bucket}_DN": -(pv_dn - pv_base) - curv_rw * sum_d,
    }
