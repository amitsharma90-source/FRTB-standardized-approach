...
def _build_ql_callable(notional, coupon, mat_date, call_date, freq, crv_handle, cfg):
    face = 100.0
    scale = notional / face
...
def calc_callable_bond_girr_delta(inst, mkt, cfg):
    ...
    bond_base, scale, _, _ = _build_ql_callable(
        ...
        inst.get('freq', 2), base_handle, cfg)
    ...
    for bt in girr_tenors:
        ...
        bond_bumped, _, _, _ = _build_ql_callable(
            inst['notional'], coupon, inst['maturity'], inst['call_date'],
            inst.get('freq', 2), bumped_handle, cfg)
...
def calc_callable_bond_csr_delta(inst, mkt, cfg):
    ...
    bond_base, scale, _, _ = _build_ql_callable(
        inst['notional'], coupon, inst['maturity'], inst['call_date'],
        inst.get('freq', 2), base_spread_handle, cfg)
    pv_base = bond_base.NPV()
...
def calc_spx_option_delta(inst, mkt, cfg):
    ...
    bump = cfg['params'].get('bsm_bump_pct', 0.01)
    bucket = inst.get('eq_bucket', 12)
    if T <= 0: return {}
    dp = (_bsm_call(S*(1+bump),K,T,r,q,sigma) - _bsm_call(S,K,T,r,q,sigma)) / bump
    return {f"EQ_{bucket}_SPX": inst['notional']*mult*dp}
...
def calc_xccy_gbp_leg_girr(inst, mkt, cfg):
    ...
    # Build GBP leg as FixedRateBond on SONIA curve
    bond, scale = _build_ql_bond(notional_usd, coupon, inst['maturity'], freq, inst.get('issue_date'))
    pv_base = _price_bond_ql(bond, base_handle, 0)
    ...
    for bt in girr_tenors:
        ...
        pv01 = (_price_bond_ql(bond, bumped_handle, 0) - pv_base) * scale / 0.0001   # MAR21.19: divide by 0.0001
        if abs(pv01) > 0.01:
            # Sign: GBP leg — bank receives GBP (long GBP bond economically)
            # Higher GBP rates → GBP PV falls → negative PV01 on GBP leg
    ...
    if abs(basis_sens) > 0.01:
        sens["GIRR_GBP_XCCY_BASIS"] = -basis_sens   # sign consistent with GBP leg
...
def calc_il_gilt_sensitivities(inst, mkt, cfg):
    ...
    bond, scale = _build_ql_bond(notional_usd, coupon, inst['maturity'], 2, inst.get('issue_date'))
    ...
    # Method A: project real CFs to nominal via breakeven π(t) = nominal(t) − real(t),
    ...
    BUMP = 0.0001
    gbp_sonia_rates  = mkt['gbp_sonia_rates']
    ...
    for cf in bond.cashflows():
        ...
        real_cf  = cf.amount() * scale                      # USD real cash flow
        pi_t     = (_interp_rate(gbp_sonia_rates, t)
                    - _interp_rate(real_rates, t))          # breakeven at this tenor
    ...
    if abs(infl_sensitivity) > 0.01:
        sens["GIRR_GBP_INFLATION"] = infl_sensitivity
