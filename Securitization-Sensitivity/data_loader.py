...
def load_market_data(filepath: str, cfg: dict) -> dict:
    ...
    # ── GBP SONIA: load raw, extend, snap to FRTB tenors ──
    gbp_raw = {0.25: 0.0430, 0.5: 0.0415}
    for label, tenor in [('SONIA_1Y', 1), ('SONIA_2Y', 2), ('SONIA_3Y', 3),
                          ('SONIA_5Y', 5), ('SONIA_10Y', 10)]:
        if label in row.index and pd.notna(row[label]):
            gbp_raw[tenor] = row[label] / 100
    gbp_raw.setdefault(7, 0.0380)
    gbp_raw.setdefault(15, 0.0410)
    gbp_raw.setdefault(20, 0.0420)
    gbp_raw.setdefault(30, 0.0425)
    mkt['gbp_sonia_rates'] = _snap_to_frtb_tenors(gbp_raw, girr_tenors)
    mkt['gbp_curve'], mkt['gbp_handle'], _, _ = _build_ql_curve(mkt['ql_val_date'], mkt['gbp_sonia_rates'])

    # ── UK Real Yield: load raw, snap to FRTB tenors ──
    uk_real_10y = row.get('UK_REAL_10Y', 0.95) / 100
    gbp_real_raw = {
        0.5: 0.0160, 1: 0.0145, 2: 0.0130, 3: 0.0120, 5: 0.0110,
        7: 0.0102, 10: uk_real_10y, 15: 0.0080, 20: 0.0065, 30: 0.0050, 50: 0.0035,
    }
    mkt['gbp_real_rates'] = _snap_to_frtb_tenors(gbp_real_raw, girr_tenors)
    mkt['gbp_real_curve'], mkt['gbp_real_handle'], _, _ = _build_ql_curve(
        mkt['ql_val_date'], mkt['gbp_real_rates'])

    mkt['uk_real_10y'] = uk_real_10y
